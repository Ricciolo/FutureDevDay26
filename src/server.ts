...
import type { InitializeResult } from "./protocol/mcpTypes.js";
import {
  definition as getTicketDef,
  call as callGetTicket,
} from "./tools/getTicket.js";
import {
  definition as getCustomerContextDef,
  call as callGetCustomerContext,
} from "./tools/getCustomerContext.js";
import {
  definition as searchKnownIssuesDef,
  call as callSearchKnownIssues,
} from "./tools/searchKnownIssues.js";
import {
  definition as getRecentLogsDef,
  call as callGetRecentLogs,
} from "./tools/getRecentLogs.js";
import type { McpToolCallResult } from "./protocol/mcpTypes.js";
  ...
  try {
    switch (request.method) {
      ...
      case "tools/call": {
        ...
        if (!toolName) {
          writeResponse(
            createErrorResponse(request.id, -32602, "Missing required parameter: name")
          );
          break;
        }
        ...
        if (!handler) {
          writeResponse(
            createErrorResponse(request.id, -32601, `Tool not found: ${toolName}`)
          );
          break;
        }
    ...
    writeResponse(
      createErrorResponse(request.id, -32603, "Internal server error", message)
    );
  }
}
...
rl.on("line", (line) => {
  ...
  if (trimmed) {
    const p: Promise<void> = handleMessage(trimmed).catch((err) => {
